...
function Roles() {
            ...
            const rolExistente = roles.find(
                (rol) =>
                    rol.descripcion.toLowerCase() ===
                        value.trim().toLowerCase() && rol.id !== rolEditando
            );
            if (rolExistente) {
    ...
    const toggleAccion = (id_menu_item, id_accion) => {
        setAccionesTmp((prev) => {
            const existe = prev.some(
                (a) =>
                    a.id_menu_item === id_menu_item && a.id_accion === id_accion
            );
            if (existe) {
                return prev.filter(
                    (a) =>
                        !(
                            a.id_menu_item === id_menu_item &&
                            a.id_accion === id_accion
                        )
                );
            } else {
    ...
    const guardarAcciones = async () => {
        ...
        const accionesRolRes = await axios.get(
            `/api/roles/${rolDetalles.id}/acciones`
        );
        setAccionesAsignadas(accionesRolRes.data);
    };
        ...
        return (
            <ul className="roles-arbol-lista">
                {items.map((menu) => {
                    ...
                    // El menú está activo si su padre está activo y él está marcado
                    const menuActivo =
                        padreActivo && menusAsignados.includes(menu.id);
                        ...
                        <li key={menu.id} className="roles-arbol-item">
                            <div className="roles-arbol-row">
                                <input
                                    ...
                                    disabled={
                                        (rolDetalles &&
                                            rolDetalles.descripcion ===
                                                "admin") ||
                                        !padreActivo // Si el padre no está marcado, deshabilita el hijo
                                    }
                                />
                                <span className="roles-arbol-texto">
                                    {menu.item}
                                </span>
                                {modoEditarAcciones &&
                                    esTerminal &&
                            ...
                            {hijos.length > 0 &&
                                renderArbol(menus, menu.id, menuActivo)}
                        </li>
                    );
                })}
            </ul>
            ...
            <div className="roles-panel-permisos">
                <div className="roles-panel-header">
                    <span className="roles-panel-titulo">
                        ...
                        <strong>{rolDetalles.descripcion}</strong>
                    </span>
                    {!modoEditarAcciones && (
                        <button
                            className="roles-editar-acciones-btn"
                            onClick={editarAccionesMenu}
                        >
                    ...
                    {modoEditarAcciones && (
                        <button
                            className="roles-guardar-acciones-btn"
                    ...
                    <button
                        className="roles-cerrar-panel-btn"
                        onClick={cerrarPanelPermisos}
                    >
                        Cerrar
                    </button>
                </div>
                <div className="roles-arbol">
                    {menusTodos &&
                        menusTodos.length > 0 &&
                        renderArbol(menusTodos, null, true)}
                </div>
                <div className="roles-panel-botones">
                    <button
                        className="roles-guardar-menus-btn"
